...
while running:
    ...
    if current_room == room_green and not chest_opened:
        message_surface = font.render("Liyue - Qiaoying Village", True, (255, 255, 255))
        message_rect = message_surface.get_rect(center=(WIDTH // 1.2, HEIGHT // 1.05))
    ...
    if current_room == room_pink:
        message_surface = font.render("Mondstadt", True, (255, 255, 255))
        message_rect = message_surface.get_rect(center=(WIDTH // 1.2, HEIGHT // 1.05))
        screen.blit(message_surface, message_rect.topleft)


    if current_room == room_blue:
        message_surface = font.render("Liyue - Plains", True, (255, 255, 255))
        message_rect = message_surface.get_rect(center=(WIDTH // 1.2, HEIGHT // 1.05))
        screen.blit(message_surface, message_rect.topleft)


    if current_room == room_tomato:
        message_surface = font.render("Dragonspine", True, (255, 255, 255))
        message_rect = message_surface.get_rect(center=(WIDTH // 1.2, HEIGHT // 1.05))
        screen.blit(message_surface, message_rect.topleft)
